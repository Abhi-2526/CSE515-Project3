...
def probabilistic_relevance_feedback(query_vector, even_vectors, even_labels, feedback_data, query_label, t):
    ...
    top_t_indices = [idx for idx, _ in scores[:t]]
...
def visualize_images(query_image_id, top_t_indices, dataset, feedback_data, t):
    feedback_positive_indices = {idx for idx, _, rating in feedback_data if rating in ['r+', 'r']}

    final_indices = [query_image_id]
    ...
    axes[0].axis('off')
    axes[0].set_title(f'Query Image: {query_image_id}')
