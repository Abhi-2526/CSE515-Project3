...
def main():
    ...
    index_file = 'lsh_index_20231128_230513.pkl'
    use_existing_index = input("Do you want to use an existing index? (yes/no): ").strip().lower()
    if use_existing_index.lower() in ['yes', 'y']:
        ...
        index_file = f'lsh_index_{current_time}.pkl'
        with open(index_file, 'wb') as f:
            pickle.dump(lsh, f)
        print(f"Index saved with name: {index_file}")

    while True:
        proceed = input("Do you want to proceed with Task 4b with index created above (Similar Image Search)? ("
                        "yes/no): ").strip().lower()
        if proceed.lower() in ['yes', 'y']:
            ...
            similar_image_indices_and_labels, unique_considered, overall_considered = lsh.find_similar_images(
                query_vector, even_vectors, even_labels, query_label, t)
